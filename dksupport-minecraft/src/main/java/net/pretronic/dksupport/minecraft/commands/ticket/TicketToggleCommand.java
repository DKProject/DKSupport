...
public class TicketToggleCommand extends BasicCommand {

    public TicketToggleCommand(ObjectOwner owner) {
        super(owner, CommandConfiguration.newBuilder()
                .name("toggle")
                .permission(DKSupportConfig.PERMISSION_STAFF).create());
    }
    ...
    private void changeLogin(OnlineMinecraftPlayer player, boolean current, boolean action){
        CommandUtil.changeLogin(PlayerSettingsKey.SUPPORT,player,current,action);
    }
}
